...
def run_elastic_net_feature_selection(
    ...
    selection_summary = (
        ...
        .agg(
            ...
            n_positive=("coefficient", lambda s: (s > 0).sum()),
            n_negative=("coefficient", lambda s: (s < 0).sum()),
        )
        .reset_index()
        .sort_values(
            ["times_selected", "mean_abs_coefficient", "feature"],
    ...
    selection_summary["selection_rank"] = range(1, len(selection_summary) + 1)
    
    selected_features_min_count = (
        selection_summary[selection_summary["times_selected"] >= min_times_selected]
        .copy()
        .reset_index(drop=True)
    )
