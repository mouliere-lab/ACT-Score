...
def run_elastic_net_feature_selection(
    ...
    train_df = data[data["split"] == "Training"].copy()
    ...
    selection_summary = (
        ...
        .sort_values(["times_selected", "feature"], ascending=[False, True])
        .reset_index(drop=True)
    )

    selected_features_min_count = selection_summary[
        selection_summary["times_selected"] >= min_times_selected
    ].copy()

    if all_selected_features:
        all_selected_features_df = pd.concat(
            all_selected_features,
            ignore_index=True,
        )
    else:
        all_selected_features_df = pd.DataFrame(
            columns=["feature", "coefficient", "abs_coefficient", "random_state"]
        )

    run_results_path = os.path.join(
        output_dir,
        f"{output_prefix}_feature_selection_run_results.csv",
    )

    selection_counts_path = os.path.join(
        output_dir,
        f"{output_prefix}_selected_feature_counts.csv",
    )

    selected_min_count_path = os.path.join(
        output_dir,
        f"{output_prefix}_selected_features_min{min_times_selected}.csv",
    )

    all_selected_path = os.path.join(
        output_dir,
        f"{output_prefix}_all_selected_features_by_random_state.csv",
    )
    ...
    logging.info(
        f"Saved all selected features by random state to {all_selected_path}"
    )
